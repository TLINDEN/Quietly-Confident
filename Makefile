# This Makefile is for the Quietly-Confident extension to perl.
#
...
#     EXE_FILES => [q[qc]]
#     LICENSE => q[perl]
#     NAME => q[Quietly-Confident]
#     PREREQ_PM => { HTTP::Server::Brick=>q[0], Git::Repository=>q[0], Moo=>q[0], Config::General=>q[0], Template=>q[0], Sub::Quote=>q[0], Text::Markdown=>q[0] }
#     VERSION_FROM => q[qc]
#     clean => { FILES=>q[t/*.out *~ */*~ */*/*~ */*/*/*~] }
...
DISTNAME = Quietly-Confident
DISTVNAME = Quietly-Confident-0.04
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  qc.pod $(INST_MAN3DIR)/qc.$(MAN3EXT) 
...
$(INST_SCRIPT)/qc : qc $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/qc
	$(CP) qc $(INST_SCRIPT)/qc
	$(FIXIN) $(INST_SCRIPT)/qc
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/qc
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - TLINDEN' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Quietly-Confident' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  Git::Repository: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  HTTP::Server::Brick: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  Moo: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  Sub::Quote: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  Template: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  Text::Markdown: 0' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "TLINDEN"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Quietly-Confident",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Git::Repository" : 0,' >> META_new.json
	$(NOECHO) $(ECHO) '            "HTTP::Server::Brick" : 0,' >> META_new.json
	$(NOECHO) $(ECHO) '            "Moo" : 0,' >> META_new.json
	$(NOECHO) $(ECHO) '            "Sub::Quote" : 0,' >> META_new.json
	$(NOECHO) $(ECHO) '            "Template" : 0,' >> META_new.json
	$(NOECHO) $(ECHO) '            "Text::Markdown" : 0' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
